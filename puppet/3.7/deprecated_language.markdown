...
Since `import` statements can interfere with syntax checking individual manifest files, you can also use [the `ignoreimport` setting](./configuration.html#ignoreimport) to block importing when using the `puppet parser validate` command.
